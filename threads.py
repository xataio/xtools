...
def producer(queue,PAGE_SIZE,from_XATA_API_KEY,from_BRANCH_URL,table,schema_links,ERROR_FILE,host_header='',mode="full",fetch_records="all"):
    ...
    while more == True:
        if mode == "full" and fetch_records == "all":
            raw_query_response,errors=post(apikey=from_XATA_API_KEY,urlPath=from_BRANCH_URL+"/tables/"+table+"/query",payload=query_payload,ERROR_FILE=ERROR_FILE,host_header=host_header)
...
def consumer(queue,reporting_queue,BULK_SIZE,to_XATA_API_KEY,to_BRANCH_URL,table,schema,schema_links,table_categories,output,output_format,output_path,ERROR_FILE,host_header='',mode="full",records_type="all"):
    ...
    while True:
        ...
        if mode=="no_links":
            for column in schema_links[table]:
                if schema_links[table][column]["link"]["table"] in table_categories["category3"]:
        ...
        if (mode == "full" and records_type=="all") or mode == "no_links":
            ...
            if len(records)==BULK_SIZE:
                if output=="xata":
                    resp,errors=post(apikey=to_XATA_API_KEY,urlPath=to_BRANCH_URL+"/tables/"+table+"/bulk",payload={"records":records},ERROR_FILE=ERROR_FILE,host_header=host_header)
        ...
        #explicitly process records with link content. This happens in the second pass for backfilling, these are reported as link counts for the reporting thread.
            ...
            if len(records)==BULK_SIZE:
                if output=="xata":
                    ...
                    resp,errors=post(apikey=to_XATA_API_KEY,urlPath=to_BRANCH_URL+"/transaction",payload=transaction_payload,ERROR_FILE=ERROR_FILE,host_header=host_header)
                    #If there are errors returned by the request, add them to the reporting queue
                    if errors != {}:
                        error_report={}
                        error_report={table:{"errors":errors}}
        ...
        #Atomic record link updates
            ...
            resp,errors=patch(apikey=to_XATA_API_KEY,urlPath=to_BRANCH_URL+"/tables/"+table+"/data/"+record_id,payload=record,ERROR_FILE=ERROR_FILE,host_header=host_header)
            if errors != {}:
                error_report={}
                error_report={table:{"errors":errors}}
                reporting_queue.put(error_report)
    ...
    if (mode == "full" and records_type == "all") or mode == "no_links":
        if output=="xata" and len(records)>0:
            resp,errors=post(apikey=to_XATA_API_KEY,urlPath=to_BRANCH_URL+"/tables/"+table+"/bulk",payload={"records":records},ERROR_FILE=ERROR_FILE)
            #If there are errors returned by the request, add them to the reporting queue
            if errors != {}:
                error_report={}
                error_report={table:{"errors":errors}}
...
def reporter(queue,tables,table_categories,from_XATA_API_KEY,from_BRANCH_URL,ERROR_FILE,CONCURRENT_CONSUMERS,host_header,output,output_path):
    ...
    if output=="xata":
        ...
        for category in table_categories:
            if category=="category1" and len(table_categories[category])>0:
                print(table_categories[category],"do not contain links and will be copied first.")
            if category=="category2" and len(table_categories[category])>0:
                print(table_categories[category],"contain links to other tables from the above list so will be copied second.")
    ...
    for table in tables:
        ...
        summaries,errors=post(apikey=from_XATA_API_KEY,urlPath=from_BRANCH_URL+"/tables/"+table+"/summarize",payload={"summaries": {"total": {"count": "*"}}},ERROR_FILE=ERROR_FILE,host_header=host_header)
        table_status[table]["records_summary"]=summaries.json()["summaries"][0]["total"]
        report[table]={}
    ...
    while True:
                ...
                if "errors" in status_report[key]:
                    ...
                    for error_code in status_report[key]["errors"]:
                        if error_code in report[key]["errors"]:
                            report[key]["errors"][error_code]+=status_report[key]["errors"][error_code]
                        else:
                            report[key]["errors"][error_code]=status_report[key]["errors"][error_code]
        except Empty:
            ...
            if "records" in report[table] and "links" not in report[table]:
                if table_status[table]["threads_finished"]!=CONCURRENT_CONSUMERS:
                    if report[table]["errors"]=={}:
                        print(table,":",report[table]["records"],"/",table_status[table]["records_summary"])
                    else:
                        print(table,":",report[table]["records"],"/",table_status[table]["records_summary"],"Errors:",report[table]["errors"])
                else:
                    if report[table]["errors"]=={}:
                        print(table,":",report[table]["records"],"/",table_status[table]["records_summary"],"[Completed]")
                    else:
                        print(table,":",report[table]["records"],"/",table_status[table]["records_summary"],"[Completed]","Errors:",report[table]["errors"])
            if "records" in report[table] and "links" in report[table]:
                if report[table]["links"]>0:
                    if table_status[table]["threads_finished"]==CONCURRENT_CONSUMERS and table_status[table]["links_finished"]==CONCURRENT_CONSUMERS:
                        if report[table]["errors"]=={}:
                            print(table,":",report[table]["records"],"/",table_status[table]["records_summary"],"[Completed]","| Backfilled links:",report[table]["links"],"[Completed]")
                        else:
                            print(table,":",report[table]["records"],"/",table_status[table]["records_summary"],"[Completed]","| Backfilled links:",report[table]["links"],"[Completed]","Errors:",report[table]["errors"])
                    elif table_status[table]["threads_finished"]==CONCURRENT_CONSUMERS and table_status[table]["links_finished"]!=CONCURRENT_CONSUMERS:
                        if report[table]["errors"]=={}:
                            print(table,":",report[table]["records"],"/",table_status[table]["records_summary"],"[Completed]","| Backfilled links:",report[table]["links"])
                        else:
                            print(table,":",report[table]["records"],"/",table_status[table]["records_summary"],"[Completed]","| Backfilled links:",report[table]["links"],"Errors:",report[table]["errors"])
                    elif table_status[table]["links_finished"]!=CONCURRENT_CONSUMERS and table_status[table]["threads_finished"]!=CONCURRENT_CONSUMERS:
                        if report[table]["errors"]=={}:
                            print(table,":",report[table]["records"],"/",table_status[table]["records_summary"],"| Backfilled links:",report[table]["links"])
                        else:
                            print(table,":",report[table]["records"],"/",table_status[table]["records_summary"],"| Backfilled links:",report[table]["links"],"Errors:",report[table]["errors"])
                else:
                    if table_status[table]["threads_finished"]!=CONCURRENT_CONSUMERS:
                        if report[table]["errors"]=={}:
                            print(table,":",report[table]["records"],"/",table_status[table]["records_summary"])
        ...
        if all_finished==True:
            ...
            if links_sum==0:
                print("Data transfer completed. Processed",records_sum,"records total.")
            elif links_sum>0:
                print("Data transfer completed. Processed",records_sum,"records total and backfilled",links_sum,"records with links.")
