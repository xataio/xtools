...
parser.add_argument("--from_XATA_API_KEY", help = "Xata API key for the source workspace",required=False)
...
parser.add_argument("--to_XATA_API_KEY", help = "Xata API key for the destination workspace",required=False)
parser.add_argument("--concurrency", help= "Number of concurrent writer threads per table. Range is 1 to 10.",required=False)
parser.add_argument("--bulk_size", help= "Number of records per write request. Range is 1 to 1000.",required=False)
parser.add_argument("--page_size",help="Number of records to fetch in each page of the scroll request. Range is 1 to 200.",required=False)
parser.add_argument("--queue_size",help="Number of inflight events we can store in the memory queue. Range is page_size to 10000.",required=False)
...
parser.add_argument("--custom_destination",help="Using a custom destination instead of Xata's Production endpoints",required=False)
parser.add_argument("--custom_destination_host_header",help="Using a custom host header for connections to the destination",required=False)
parser.add_argument("--custom_control_plane",help="Using a custom control plane address",required=False)
...
if not args.from_XATA_API_KEY:
    try:
        ...
        print("from_XATA_API_KEY must be passed as argument or set as env variable in order to access the source workspace")
else:
...
if OUTPUT=="xata":
    if not args.to_XATA_API_KEY:
        try:
            ...
            print("to_XATA_API_KEY must be passed as argument or set as env variable in order to access the destination workspace")
            exit(-1)
    else:
    ...
    if not args.output_path:
        OUTPUT_PATH="output/"+FROM_WORKSPACE+"-"+FROM_DATABASE+"-"+FROM_REGION+"-"+FROM_BRANCH+"-"+str(datetime.now().isoformat(timespec="seconds"))+"/"
    else:
...
if not args.bulk_size:
    ...
    if BULK_SIZE > 500:
        print("Warning: You have configured a large bulk size. If there are large records in the tables to copy, you could reach the transaction size limit. In case errors occur, consider retrying with a smaller bulk size.")
else:
...
if not args.queue_size:
    ...
    print("Error: Queue size should be between the page size of",PAGE_SIZE,"and the limit of 10000.")
    exit(-1)

if not args.error_file:
    ERROR_FILE="logs/debug-"+TO_WORKSPACE+"-"+TO_DATABASE+"-"+TO_BRANCH+"-"+str(datetime.now().isoformat(timespec="seconds"))+".log"
else:
...
if (args.custom_source):
    from_BRANCH_URL=str(args.custom_source)+"/db/"+FROM_DATABASE+":"+FROM_BRANCH
    if (args.custom_source_host_header):
        source_host_header=str(args.custom_source_host_header)
    else:
        source_host_header=FROM_WORKSPACE+"."+FROM_REGION+"."+DEFAULT_BASE_URL_DOMAIN
else:
    from_BRANCH_URL="https://"+FROM_WORKSPACE+"."+FROM_REGION+"."+DEFAULT_BASE_URL_DOMAIN+"/db/"+FROM_DATABASE+":"+FROM_BRANCH
    source_host_header=''

if (args.custom_destination):
    to_BRANCH_URL=str(args.custom_destination)+"/db/"+TO_DATABASE+":"+TO_BRANCH
    if (args.custom_destination_host_header):
        destination_host_header=str(args.custom_destination_host_header)
    else:
        destination_host_header=TO_WORKSPACE+"."+TO_REGION+"."+DEFAULT_BASE_URL_DOMAIN
else:
    to_BRANCH_URL="https://"+TO_WORKSPACE+"."+TO_REGION+"."+DEFAULT_BASE_URL_DOMAIN+"/db/"+TO_DATABASE+":"+TO_BRANCH
    destination_host_header=''

if (args.custom_control_plane):
    to_DB_MANAGEMENT_URL=str(args.custom_control_plane)+"/workspaces/"+TO_WORKSPACE+"/dbs/"+TO_DATABASE
else:
    to_DB_MANAGEMENT_URL="https://"+DEFAULT_CONTROL_PLANE_DOMAIN+"/workspaces/"+TO_WORKSPACE+"/dbs/"+TO_DATABASE

#verify the origin branch exists
branch_check_response,errors=get(apikey=from_XATA_API_KEY,urlPath=from_BRANCH_URL,expect_codes=[],ERROR_FILE=ERROR_FILE)
if branch_check_response.status_code != 200:
    print("Aborting because the origin branch "+str(from_BRANCH_URL)+" was not fround: ",branch_check_response)
    exit(-1)
...
if OUTPUT=="xata":
    ...
    if FROM_WORKSPACE!=TO_WORKSPACE or FROM_DATABASE!=TO_DATABASE:
        db_create_response,errors=put(apikey=to_XATA_API_KEY,urlPath=to_DB_MANAGEMENT_URL,expect_codes=[422],payload={"region": TO_REGION,"branchName": TO_BRANCH},ERROR_FILE=ERROR_FILE)
        if db_create_response.status_code==422:
            print("Aborting because the target database",TO_DATABASE,"already exists in workspace",TO_WORKSPACE)
            exit(-1)
        target_type="new_database"
    #If we are writting to the same database we require the target branch does not exist
    elif FROM_WORKSPACE==TO_WORKSPACE and FROM_DATABASE==TO_DATABASE and FROM_BRANCH!=TO_BRANCH:
        if FROM_REGION!=TO_REGION:
            print("Origin and target region must match when origin and target database is the same.")
            exit(-1)
        branch_check_response,errors=get(apikey=to_XATA_API_KEY,urlPath=to_BRANCH_URL,expect_codes=[],ERROR_FILE=ERROR_FILE,host_header=destination_host_header)
        if branch_check_response.status_code!=404:
            print("Aborting because we couldn't verify the target branch "+str(to_BRANCH_URL)+" does not exist: ",branch_check_response)
            exit(-1)
        target_type="same_database_new_branch"
    #Origin and target branch should not match
    else:
        print("Aborting, origin and target branch cannot be the same."," Origin: ",FROM_WORKSPACE,FROM_DATABASE,FROM_BRANCH," Target: ",TO_WORKSPACE,TO_DATABASE,TO_BRANCH)
        exit(-1)
#################
...
print(" Concurrent writers per table:",CONCURRENT_CONSUMERS,"\n Bulk size:",BULK_SIZE,"\n Page scroll size:",PAGE_SIZE,"\n Inflight records queue size:",MAX_QUEUE_SIZE,"\n Error logs written to:",ERROR_FILE)

#Copy schema
print("\nApplying schema:")
print("- Retrieving schema from origin",from_BRANCH_URL)
from_schema_raw,errors=get(apikey=from_XATA_API_KEY, urlPath=from_BRANCH_URL,ERROR_FILE=ERROR_FILE,host_header=source_host_header)
...
for table in from_schema["schema"]["tables"]:
    ...
    for column in table["columns"]:
        columns_migration["operations"].append({"addColumn":{"table":table["name"],"column":column}})
...
if OUTPUT=="xata":
    ...
    if target_type=="new_database":
        #############################
        tables_migration_response,errors=post(apikey=to_XATA_API_KEY,urlPath=to_BRANCH_URL+"/schema/update",payload=tables_migration,ERROR_FILE=ERROR_FILE,host_header=destination_host_header)
        if tables_migration_response.json()["status"]!="completed":
            print("Error when creating tables. Unexpected status in /schema/update API response.")
            exit(-1)
        columns_migration_response,errors=post(apikey=to_XATA_API_KEY,urlPath=to_BRANCH_URL+"/schema/update",payload=columns_migration,ERROR_FILE=ERROR_FILE,host_header=destination_host_header)
        if columns_migration_response.json()["status"]!="completed":
            print("Error when applying schema to tables. Unexpected status in /schema/update API response.")
            exit(-1)
        #############################
    #Create database branch from existing branch
    elif target_type=="same_database_new_branch":
        from_branch_payload={"from":FROM_BRANCH}
        branch_create_response,errors=put(apikey=to_XATA_API_KEY,urlPath=to_BRANCH_URL,payload=from_branch_payload,expect_codes=[],ERROR_FILE=ERROR_FILE,host_header=destination_host_header)
        if branch_create_response.status_code!=201 or branch_create_response.json()["status"]!="completed":
            print("Error while creating target branch "+to_BRANCH_URL+" from origin "+FROM_BRANCH+" :",branch_create_response)
            exit(-1)
    else:
        print("Aborting: Could not identify the target branch. It does not appear to be a new branch in a new or an existing database.")
        exit(-1)

    to_schema_raw,errors=get(apikey=to_XATA_API_KEY, urlPath=to_BRANCH_URL,ERROR_FILE=ERROR_FILE,host_header=destination_host_header)
